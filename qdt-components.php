<?php
	...
	*/
    ...
    define( 'QDT_COMPONENTS_PLUGIN_VERSION', '0.1.3' );
    ...
    wp_register_script( 'qdt-components-js', QDT_COMPONENTS_PLUGIN_PLUGIN_DIR . 'scripts/index.js', array('jquery'), QDT_COMPONENTS_PLUGIN_VERSION, $in_footer = true );
	...
	function qdt_components_plugin_menu() {
		add_menu_page( esc_attr__('Qdt-components Plugin Settings', 'qdt-components'), 'Qdt-components', 'administrator', 'qdt_components_plugin_settings', 'qdt_components_plugin_settings_page', QDT_COMPONENTS_PLUGIN_PLUGIN_DIR . 'qlik.png', null );
    }
	...
	function qdt_components_plugin_settings_page() {
		...
		<div class="wrap">
			...
			<form method="post" action="options.php">
				...
				<div style="border-top:1px solid #ccc;padding-top:35px;"><a href="https://www.qlik.com/us/"><img src="<?php echo QDT_COMPONENTS_PLUGIN_PLUGIN_DIR . "QlikLogo-RGB.png"?>" width="200"></a></div>
			</form>
		</div>
		<?php
    }
	...
	// [qdt-component type="QdtKpi" id="kpi1" expr="=Num(Count(date_demobilization), '###,###')"]
	function qdt_component_func( $atts ) {
		wp_enqueue_script( 'qdt-components-js-lib' );
		wp_enqueue_script( 'qdt-components-js' );
        
        if ($atts['type'] === 'QdtViz') {
            return "<div id=\"qdt_{$atts['id']}\" class=\"wp-qdt\" data-type=\"{$atts['type']}\" data-id=\"{$atts['id']}\" data-height=\"{$atts['height']}\"></div>";
        } else if ($atts['type'] === 'QdtKpi') {
            return "<div id=\"qdt_{$atts['id']}\" class=\"wp-qdt\" data-type=\"{$atts['type']}\" data-id=\"{$atts['id']}\" data-expr=\"{$atts['expr']}\"></div>";
        }
	}
